...
function PerfilCapilarTab({ client }: { client: Cliente }) {
  ...
  return (
    <>
      ...
      <div className="glw-cap-grid">
        <div className="glw-cap-card">
          <div className="glw-cap-card-label">Porosidad</div>
          <div className="glw-cap-card-value">{datos?.porosidad || '—'}</div>
          <div className="glw-cap-card-desc">
            {datos?.porosidad === 'Alta' ? 'Absorbe y pierde humedad rápido.'
              : datos?.porosidad === 'Baja' ? 'Resistente a absorber producto.'
              : 'Absorción equilibrada.'}
          </div>
        </div>
        <div className="glw-cap-card">
          <div className="glw-cap-card-label">Densidad</div>
          <div className="glw-cap-card-value">{datos?.densidad || '—'}</div>
        </div>
        <div className="glw-cap-card">
          <div className="glw-cap-card-label">Elasticidad</div>
          <div className="glw-cap-card-value">{datos?.elasticidad || '—'}</div>
        </div>
        <div className="glw-cap-card">
          <div className="glw-cap-card-label">Estado del cuero</div>
          <div className="glw-cap-card-value">{datos?.oleosidad || datos?.exterior_lipidico || '—'}</div>
        </div>
      </div>
...
export function ClientDetail({ client, isOpen, onClose, onClientUpdated }: ClientDetailProps) {
  ...
  const tabs: { key: Tab; label: string }[] = [
    ...
    { key: 'perfil', label: 'Perfil Capilar' },
